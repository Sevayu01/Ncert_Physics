      ...
      {subtopics.map((e, idx) => {
        return (
          <Link to={`/${chapter}/${e.link}`} key={idx}>
            <p className="text-left text-[15px] pb-3 hover:text-blue-800 leading-5">{e.name}</p>
          </Link>
        );
      })}
      <Link to={`/${chapter}/quiz`} key={8}>
        <p className="text-xl pb-2 bg-[#556672] hover:bg-[#3c4851] w-[80%] text-center rounded-md items-center py-2 text-white font-medium">Quiz</p>
      </Link>
    </div>
  );
};
